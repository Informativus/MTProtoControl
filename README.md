...
- если ты хочешь использовать `private_key_path`, положи SSH-ключи в папку `./ssh` рядом с release `docker-compose.yml`; эта папка монтируется в контейнер API только на чтение как `/root/.ssh`
