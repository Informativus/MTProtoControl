...
- каталог с SSH-ключами можно примонтировать в контейнер API только на чтение, если ты хочешь использовать `private_key_path` вместо пароля или вставленного ключа
...
- `SSH_KEY_DIR`: путь на хосте до каталога с SSH-ключами для режима `private_key_path`; внутри контейнера он будет доступен как `/root/.ssh`. Если используешь пароль или вставленный ключ, можно оставить `./ssh`
